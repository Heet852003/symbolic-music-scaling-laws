"""
Training script for RNN models.
"""
...
def train_epoch(model, train_loader, optimizer, scheduler, device, config, save_step_losses=False):
    ...
    for batch_idx, (x, y) in enumerate(pbar):
        ...
        # Forward pass
        logits, loss, hidden = model(x, y, hidden=hidden)
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='outputs/models/rnn',
    ...
    # Get model config for specified size
    model_config = config['models'][args.model_size]
    ...
    # Create model
    model = create_rnn_model(vocab_size, model_config)
